...
import { enricherAgent } from "@/mastra/agents/enricher";
...
export const enrichStep = createStep({
  id: "enrich-step",
  description: "Enrich the code with additional information",
  inputSchema: z.void(),
  outputSchema: z.void(),
  execute: async ({
    runtimeContext,
    ...
    const contextVersion = runtimeContext.get("version");
    ...
    await db.transaction(async (tx) => {
      ...
      for (const file of changedFiles) {
        ...
        const agentResponse = await enricherAgent.generate(
          [
            {
              role: "user",
              content: `# Code
          ...
          {
            output: enricherAgentOutputSchema,
            runtimeContext,
          },
        );
        ...
        for (const declaration of insertDeclarations) {
          ...
          const storedPkgs = await tx.query.packages.findMany({
            where: and(
              inArray(
                packages.name,
                enrichedDeclaration.dependencies.external.map(
                  (pkg) => pkg.name,
                ),
              ),
              ...
              (declaration) => {
                if (!dependency.importPath) {
                  ...
                  if (
                    ...
                    const endpointSpecs = declaration.declaration.specs
                      .data as {
                      definition: {
                        name?: string;
                        method?: string;

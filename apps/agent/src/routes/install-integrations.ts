...
router.openapi(route, async (c) => {
  ...
  // Build integration config from completed integrations
  ...
  try {
    logger.info("Starting integration installation process");

    const context = new WorkflowContext();
    context.set("project", { ...project, config: integrations });

    await processIntegrationQueue(context);

    const result = await installQueuedIntegrations(context);

...
import { and, db, eq, sql } from "..";
import type { updateFunctionSchema, updateRouteFlowSchema } from "../schema";
...
// TODO: many of these queries should be optimized
...
export async function getFunctionPrimitiveWithSecretsById({
  ...
  id: string;
}): Promise<Omit<FunctionData, "rawDescription"> | undefined> {
  const result = await db.query.primitives.findFirst({
    where: and(eq(primitives.id, id), eq(primitives.type, "function")),
  });
